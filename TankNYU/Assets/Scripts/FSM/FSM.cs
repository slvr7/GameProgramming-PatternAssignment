...
public class FSM : MonoBehaviour
{
    ...
    void Disable()
    {
        AITransform.gameObject.SetActive(false);
    }
    ...
    void FSMUpdate()
    {
        ...
        if (currentState != FSMState.Dead && !AIHealth.IsAlive)
        {
            currentState = FSMState.Dead;
        }
    }
    ...
    private void caculateStopTime()
    {
        ...
        {
            previousPos = AITransform.position;
        }
        else
        {
            Vector3 posDiff = AITransform.position - previousPos;
    ...
    void UpdateWanderState()
    {
        ...
        targetPlayer = AISensor.getNearbyPlayer();
        ...
        if (AgentDone())
        {
            ...
            Vector3 nextDestination = AITransform.position + randomRange;
        ...
        if (stopTime > 1.0f)
        {
            Vector3 nextDestination = AITransform.position
                - AITransform.forward * (Random.value) * wanderScope;
            agent.destination = nextDestination;
        }
    ...
    void UpdateSeekState()
    {
        ...
        if (AIHealth.getDamaged)
        {
            Vector3 seekDirection = AIHealth.damageDirection;
            agent.destination = AITransform.position
                + seekDirection * seekDistance;

            AIHealth.getDamaged = false;
        }
    ...
    void UpdateChaseState()
    {
        ...
        if (Vector3.Distance(targetPlayer.position, AITransform.position) <= attackRange)
        {
    ...
    void UpdateAttackState()
    {
        ...
        if (ph != null)
        {
            ...
            Vector3 direction = targetPlayer.position - AITransform.position;
            float degree = Vector3.Angle(direction, AITransform.forward);
            if (degree < attackFieldOfView / 2 && degree > -attackFieldOfView / 2)
            {
                ...
                if (attackTimer > attackInterval)
                {
                    ...
                    if (zombieAttackAudio != null)
                        AudioSource.PlayClipAtPoint(zombieAttackAudio, AITransform.position);
            ...
            else
            {
                ...
                animator.SetBool("isAttack", false);
                AITransform.LookAt(targetPlayer);
            }
        }
